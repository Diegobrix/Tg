      ...
      <main>
         <section>
            <div class="recipes_overview--wrapper">
               ...
               <div class="popular_category--wrapper">
                  ...
                  <p class="popular_category-display"><?=$mostPopularCategory != null?$mostPopularCategory['title']:''?></p>
                  <div>
                     <p class="popular_category_amount-display"><?=$mostPopularCategory != null?$mostPopularCategory['amount']:''?></p>
                     <span>Receitas</span>
                  </div>
               </div>
               <div class="most_recent_recipe--wrapper">
                  <h2>Última Receita Adicionada</h2>
                  <p class="most_recent_recipe-display"><?=$lastRecipe!=null?$lastRecipe['title']:''?></p>
                  <figure>
         ...
         <section class="other_options-wrapper">
            <div class="videos-widget">
               <?php
                  $videoFilePath = "./data/datasets/videos.json";
                  $categoryFilePath = "./data/datasets/categories.json";
                  $videos = 0;
                  $categories = 0;

                  if(file_exists($videoFilePath))
                  {
                     $videos = json_decode(file_get_contents($videoFilePath), true);
                  }
                  
                  if(file_exists($categoryFilePath))
                  {
                     $categories = json_decode(file_get_contents($categoryFilePath), true);
                  }
               ...
               <span>Sugestões do Dia</span>
                  <div class="suggestions--wrapper">
                     ...
                     <?php
                           ...
                           {
                     ?>
                           <div class="suggestion" data-current_step="<?=$i?>" id="suggestion_<?=$i?>" style="" aria-current="<?=$i==0?'true':'else'?>">
                              <a class="suggestion_handler-prev suggestion_handler<?=$i==0?' first':''?>" href="#suggestion_<?=$i-1?>"><</a>
                              <p class="suggestion_title"></p>
                              <a class="suggestion_handler-next suggestion_handler<?=$i>=sizeof($daySuggestions)-1?' last':''?>" href="#">></a>
                           </div>
                     <?php
                           }
                        }
